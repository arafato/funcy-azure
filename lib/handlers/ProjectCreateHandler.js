...
class ProjectCreateHandler {
	...
	*/
	...
	_createCloudEnv() {
		...
		vars += `AzureWebJobsStorage=[concat('DefaultEndpointsProtocol=https;AccountName=', variables('storageAccountName'), ';AccountKey=', listKeys(variables('storageAccountid'),'2015-05-01-preview').key1)]` + os.EOL;
        vars += `AzureWebJobsDashboard=[concat('DefaultEndpointsProtocol=https;AccountName=', variables('storageAccountName'), ';AccountKey=', listKeys(variables('storageAccountid'),'2015-05-01-preview').key1)]` + os.EOL;
        vars += `WEBSITE_CONTENTAZUREFILECONNECTIONSTRING=[concat('DefaultEndpointsProtocol=https;AccountName=', variables('storageAccountName'), ';AccountKey=', listKeys(variables('storageAccountid'),'2015-05-01-preview').key1)]` + os.EOL;
