...
class ProjectCreateHandler {
		...
		return BbPromise.try(() => {
			...
			return fs.mkdirsAsync(this.projectFolder)
				...
				.then(BbPromise.join(
					...
					(() => {
						return this._modifyVersionControl(templateFolder, projectName, vcLocation)
							.then(() => {
	...
	*/
	...
	_modifyVersionControl(templateFolder, projectName, location) {
		if (location === 'local') {
			return BbPromise.join(
				io.readModWriteAsync(path.join(this.projectFolder, 'f-resources-arm.json'), path.join(this.projectFolder, 'f-resources-arm.json'), (template) => {
					template.resources[1].resources[2].properties.scmType = 'LocalGit';
					return template;
				}),
				io.readModWriteAsync(path.join(this.projectFolder, 'f-project.json'), path.join(this.projectFolder, 'f-project.json'), (data) => {
					data.versionControl.location = projectName + '.scm.azurewebsites.net';
					return data;
				}));
		} else {
