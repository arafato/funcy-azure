        ...
        env.getAdminVars()
            .then((creds) => {
                ...
                if (username === undefined || password === undefined) {
                    console.log(chalk.red('No deployment credentials have been specified (neither in admin.env nor via CLI). Log output is thus deactivated.'));
                    console.log(chalk.red('Be sure that'))
                } else {
                    url = `https://${username}:${password}@${project.name}.scm.azurewebsites.net/logstream`;
                    request.get(url).pipe(new LogPrefix('[LOG]')).pipe(process.stdout);

                    // Fetch and store authkey (masterkey) in admin.env in case no key has been specified via CLI or admin.env
                    if (authkey === undefined) {
                        console.log(chalk.cyan('[FAZ] Fetching and caching authorization secrets in admin.env...'))
            ...
            .then(() => {
                console.log(chalk.cyan(`[FAZ] Triggering ${fName}...`));
            ...
            .then((data) => {
                if (data === undefined) { // non-http triggered, no output 
                    console.log(chalk.cyan(`[FAZ] Successfully triggered ${fName}.`));
                } else {
                    console.log(chalk.cyan(`[FAZ] OUTPUT: ${data}.`));
        ...
        return BbPromise.try(() => {
            ...
            if (trigger.type === Events.TRIGGER.HTTP_TRIGGER) {
                if (args.bindings.req.method === 'GET' || !args.bindings.req.method) { // GET per default
                    queryParams = this._convertQueryParams(args.bindings.req.query);
                ...
                // Search for trigger.name in args.bindings, get value and pass as to options.body as {input:value}
            ...
            return env.getAdminVars()
                .then((creds) => {
                    options.method = 'POST';
                    options.uri = `https://arafato-func-test.azurewebsites.net/admin/functions/${fName}${queryParams}`;
                    options.headers = { "Content-Type": "application/json", "x-functions-key": creds.masterKey };
                    options.json = true;
                    return options;
                });
        });
