...
class ProjectProvisionHandler {
		...
		return BbPromise.try(() => {
			...
			return avp.then((creds) => {
				...
				this._updateARMTemplate()
					...
					.then((res) => {
						// Getting http-triggered endpoints
						spinner.stop();
						console.log(chalk.green('DONE!'));
						spinner = new Spinner(chalk.cyan('Getting http-triggered endpoints for your convenience... %s'));
						spinner.setSpinnerString('|/-\\');
						spinner.start();
						return this._getHttpTriggeredEndpoints(creds, resourceGroup, project.name);
					...
					.then((endpoints) => {
						...
						if (endpoints.length === 0) {
							console.log(chalk.cyan('No http-triggered endpoints found.'));
						} else {
							console.log(chalk.cyan(`	Found ${endpoints.length} http-triggered endpoint(s):`));
							endpoints.forEach((f) => {
								console.log(chalk.cyan('	>>>  ' + f.trigger_url));
							});
						}
	...
	_getHttpTriggeredEndpoints(creds, rg, projectname) {
		return arm.acquireAccessTokenBySPN(creds.tenant_id, creds.app_id, creds.password)
			.then((token) => {
				return arm.listFunctions(creds.subscription_id, rg, projectname, token)
					.then((functions) => {
						let httpFunctions = [];
						functions.value.forEach((f, i) => {
							f.properties.config.bindings.forEach((b) => {
								if (b.type === Types.HTTP_TRIGGER) {
					...
					.then((httpFunctions) => {
						...
						httpFunctions.forEach((fName) => {
							promises.push(arm.getFunctionSecret(creds.subscription_id, rg, projectname, fName, token));
						});
						return BbPromise.all(promises)
					});
			})
			.catch((e) => {
				throw new FError('There was a problem getting the http-triggered endpoints.');
			})
	}
