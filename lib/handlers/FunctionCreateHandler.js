...
class FunctionCreateHandler {
	...
	process(options) {
		return BbPromise.try(() => {
			...
			let connectionStringKey = path.basename(projectFolder) + '_' + fName + '_' + eventTrigger; 
	...
	_appendEnvAsync(connectionStringKey, connectionString, projectFolder) {
		...
		let env = connectionStringKey + '=' + connectionString + os.EOL;
		return fs.appendFileAsync(path.join(projectFolder, 'cloud.env'), env);
	}
