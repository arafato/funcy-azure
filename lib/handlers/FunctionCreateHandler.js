...
class FunctionCreateHandler {
		...
		return BbPromise.try(() => {
			...
			return fs.mkdirsAsync(functionFolder)
				.then(BbPromise.join(
					...
					this._appendEnvAsync(connectionString, projectFolder),
					() => {
	...
	help() {
		...
		help += '      The connection string to authorize against the event source (not needed for http event)' + os.EOL;
		console.log(chalk.cyan(help));
	}
	
	_appendEnvAsync(connectionString, projectFolder) {
		let env = this.connectionStringKey + '=' + connectionString;
	...
	_createFunctionJson(path, eventTrigger, connectionStringKey) {
		return BbPromise.try(() => {
			...
			return fs.writeFileAsync(path, JSON.stringify(o));
		});
	}
}
